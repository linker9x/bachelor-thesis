...
def read_sum_file(csv_path, out_path, dataset):
    ...
    for clf_name in sum_dict.keys():
        ...
        plot_df = plot_df.T
        plot_out_path = csv_path.replace('_sum_result.csv','/' + dataset + '_' + clf_name + '_rplot.png')
        fig = plt.figure()
        ax = fig.add_subplot(111)
        plot_df.boxplot(fontsize ='small', figsize=(19,10),
...
def gen_rplot_all(dir):
    ...
    for file in files:
        ...
        read_sum_file(dir + file, dir + file.replace('_sum_result.csv', '_stats.csv'), dataset)
