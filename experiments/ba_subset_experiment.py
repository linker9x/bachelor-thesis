...
percents = [.02, .04, .06, .08, .1, .15, .2, .3, .4, .5, .6, .7, .8, .9, 1.0]


def run_subset_experiment(alg_name, ds_names):
    # datasets to use
    ds_names = ds_names
        ...
        for train_index, test_index in cv.split(X, y):
            ...
            if alg_name not in param_methods:
                ...
                if alg_name in filter_methods:
                    ...
                    print(feat_len)
                    new_vals = embedded_fs(alg_name, X_train, y_train, feature_names, num_features=feat_len, hsic_data=train_filepath)
            ...
            for per in percents:
                ...
                if alg_name in param_methods:
                    ...
                    if alg_name in filter_methods:
                        ...
                        new_vals = embedded_fs(alg_name, X_train, y_train, feature_names, hsic_data=train_filepath)
                    elif alg_name in wrapper_methods:
                        new_vals = wrapper_fs(alg_name, X_train, y_train, feature_names)
                    elif alg_name in dl_methods:
                        new_vals = deepl_fs(alg_name, X_train, y_train, feature_names)
                ...
                num_feat = int(per * len(new_vals[alg_name]))
                if num_feat == 0:
                    num_feat = 1
                selected_feats = new_vals[alg_name][:num_feat]
                print('No. Feats:', num_feat, '\n')

                # save the selected features to the dict on last iteration for ranked algs
                if alg_name not in param_methods:
                    if per == percents[-1]:  # last element
                ...
                # if this is a method that returns a subset or takes a param, do this every iter
                    ...
                    selected_feats_dict[alg_name][fold_idx].append('NO_FEAT_' + str(per) + '_' + str(num_feat))
                ...
                if per not in fold_returned_dict[alg_name]:
                    fold_returned_dict[alg_name][per] = list()
                # save selected feats for number of feats to dict
                fold_returned_dict[alg_name][per].append(selected_feats)
...
def write_results(results, alg_name, dir_out, selected_feats_dict):
    ...
    writers[alg_name].write('alg_name,fold_id,per_feat,clf_name,acc,pre,rec,f1,class_pre,class_rec,class_f1\n')
    # for every fold
    for fold_idx in results.keys():
        ...
        for per in sorted(alg_fold_res.keys()):
            feat_res = alg_fold_res[per]
            if feat_res == {}:
                continue
            # write data for each entry
            for clf_name in feat_res.keys():
                writers[alg_name].write(str(alg_name) + ',' + str(fold_idx) + ',' + str(per) + ',' + str(clf_name))
