...
def evaluate_reconstruction(X_train, selected_X_train, X_test, selected_X_test, num_feat):
    # kf = KFold(n_splits=5, shuffle=True)
    # losses = []
    # for train_index, test_index in kf.split(X):
    #     model = Sequential()
    #     model.add(Dense(10, input_dim=num_feat, activation = 'relu'))
    #     model.add(Dense(X.shape[1]))
    #     model.compile(optimizer='Adam', loss='mse')
    #     X_train, X_test = X[train_index], X[test_index]
    #     selected_X_train, selected_X_test = selected_X[train_index], selected_X[test_index]
    #     model.fit(selected_X_train, X_train, epochs=500, verbose=0)
    #     losses.append(model.evaluate(selected_X_test, X_test))
    #     K.clear_session()
    # gc.collect()
    # return [np.mean(losses), np.std(losses), 0.0, 0.0]
    model = Sequential()
    model.add(Dense(10, input_dim=num_feat, activation = 'relu'))
    model.add(Dense(X_train.shape[1]))
    model.compile(optimizer='Adam', loss='mse')
    model.fit(selected_X_train, X_train, epochs=200, verbose=0)
    losses = model.evaluate(selected_X_test, X_test)
    K.clear_session()
    gc.collect()
    return [losses, 0.0, 0.0, 0.0, 0.0]
