from dl_methods.methods.TSFS.RRFS import RRFS
...
import numpy as np
from dl_methods.methods.TSFS.mnist_model import getCodes
...
from dl_methods.methods.TSFS.AEFS_final import AEFS
...
def my_autoencoder(X, y=None, l1 = .1, n_components=2, **kwargs):
    ...
    layer1 = Dense(10, activation='relu')(input_tensor)
    l2 = Dense(n_components, activation='relu')(layer1)
    ...
    print(X.shape)
    print(codes)
    score = rrfs.train_fs_network(X.astype(float),rep=codes.astype(float), l1=l1, epochs=300, loss='mse')
    # sort the feature scores in an ascending order according to the feature scores
    idx = np.argsort(score)[::-1]
    return idx
