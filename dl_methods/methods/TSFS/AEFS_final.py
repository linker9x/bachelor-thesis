...
from dl_methods.methods.TSFS.utils import getSyntheticDataset
...
class AEFS():
    ...
    def layer1_reg(self, weight_matrix):
        return self.alpha * K.sum(K.sqrt(tf.reduce_sum(K.square(weight_matrix), axis=1))) + (self.beta / 2.) * K.sum(
            K.square(weight_matrix))
